    ...
    <div>
        ...
        <svg width="300" height="300">
            <g transform="translate(50, 50)">
                <circle :r="clock.cw / 2" stroke="#ccc" fill="none" :transform="`translate(${clock.cw / 2}, ${clock.ch / 2})`" />
                <g
                ...
                />
                <circle r="4" stroke="#999" fill="#ccc" :transform="`translate(${clock.cw / 2}, ${clock.ch / 2})`" />
...
export default class PageSvg extends Vue {
    clock: any = {
        c: null, // 当前时间
        h: 0,
        m: 0,
    ...
    parseToDeg (num: number, type: string) {
        let dg = 60
        let n = num
        const px = -180
        if (type === 'h') {
    ...
    parseToCalibration (index: number, dg: number = 60) {
        let w = 1
        let h = 2
        let c = '#ccc'
        let ih = false
