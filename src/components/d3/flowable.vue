<template>
    <div>
        <svg id="svgTree" width="420" height="300"></svg>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        initTree () {
            ...
            // let nodes = tree.nodes(city_tree)
            //
            ...
            link.attr('d', d => {
                return (
                    ...
                    d.source.x +
                    ',' +
                    d.source.y + options.boxHeight +
                    'V' +
                    ((targetY - d.source.y + options.boxHeight) / 2 + d.source.y + options.boxHeight) +
                    'H' +
                    d.target.x +
                    'V' +
                    d.target.y
            ...
            node
                .append('rect')
                .attr('y', 0)
                .attr('x', d => d.depth !== 2 ? -(options.boxWidth / 2) : -(options.boxHeight / 2))
                .attr('width', d => d.depth !== 2 ? options.boxWidth : options.boxHeight)
                .attr('height', d => d.depth !== 2 ? options.boxHeight : options.boxWidth)
                .attr('fill', '#fff')
            ...
            node
                .append('text')
                .attr('y', d => d.depth !== 2 ? options.boxHeight / 2 + 5 : 0)
                .attr('style', d => d.depth !== 2 ? '' : 'writing-mode: tb;letter-spacing:0px')
                .attr('text-anchor', d => d.depth !== 2 ? 'middle' : 'start')
                .text( d => d.name)
        }
    }
}
